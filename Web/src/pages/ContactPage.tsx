...
const ContactPage: React.FC = () => {
  ...
  // Fetch user messages with admin replies
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { ...formData, subject: subjectMapping[formData.subject] || formData.subject },
      ...
      if (response.data?.success) {
        ...
        fetchMessages(); // refresh messages including admin replies
      } else {
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
            ...
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start">
              ...
              <div>
                <p className="text-sm text-yellow-700">
                  ...
                  <button onClick={() => navigate('/login?redirect=/contact')} className="text-blue-600 hover:underline font-medium">log in</button>{' '}
                  or{' '}
                  <button onClick={() => navigate('/register?redirect=/contact')} className="text-blue-600 hover:underline font-medium">register</button>{' '}
                  to continue.
                </p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              {formErrors.emailAddress && <p className="mt-1 text-sm text-red-600">{formErrors.emailAddress}</p>}
            </div>
            ...
            <button type="submit" disabled={isSubmitting || (!isAuthenticated && !formData.message)}
          ...
          {isAuthenticated && (
            <div className="mt-8">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">Your Messages & Replies</h3>
              {loadingMessages ? (
                ...
                <div className="space-y-6">
                  {messages.map(msg => (
                    <div key={msg._id} className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                      <p className="text-gray-700"><strong>Subject:</strong> {msg.subject}</p>
                      <p className="text-gray-700 mt-1"><strong>Message:</strong> {msg.message}</p>
                      <p className="text-gray-500 text-sm mt-1">Sent on: {new Date(msg.createdAt).toLocaleString()}</p>
                      {msg.replies.length > 0 && (
                        <div className="mt-3 space-y-2 border-t border-gray-200 pt-2">
                          {msg.replies.map(reply => (
                            <div key={reply._id} className="p-2 bg-blue-50 rounded-lg">
                              <p className="text-gray-800"><strong>Admin ({reply.admin.name}):</strong> {reply.message}</p>
                              <p className="text-gray-500 text-sm">Replied on: {new Date(reply.createdAt).toLocaleString()}</p>
                            </div>
                          ))}
                        </div>
                      )}
