...
//   // Assume vehicle data is passed via location.state (from previous page)
...
const EditVehicleDetails: React.FC = () => {
  ...
  // prefer vehicle passed via navigation state, fall back to mockVehicles by id, then default
  const vehicleFromState = (location.state as any)?.vehicle;
  const vehicleFromMock = id
    ? mockVehicles.find((v) => v._id === id)
    : undefined;
  ...
  // if route param provided and mock lookup found a vehicle after mount,
  // ensure vehicleData reflects it (covers cases where location.state is empty)
  useEffect(() => {
    if (!vehicleFromState && vehicleFromMock) {
      setVehicleData(vehicleFromMock);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id]);
